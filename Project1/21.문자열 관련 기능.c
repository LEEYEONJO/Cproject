...
int main()
{
   ...
   //name = "파이썬"; //배열은 수정 불가
	...
	job = "Programmer"; //가리키는 값이 변경되는 것이므로, 가능
	...
	//함수 3. strcpy()
	//strcpy(바꿀 문장, 바꾸려는 문장, 개수);를 통해 문자열을 바꾸려는 문장 중 일부의 값만 가져오는게 가능합니다.
	char sample3[] = "Hello Everyone";
	char sample4[20];
	strcpy(sample4, sample3, 5); //비어있는 값에다 복사해줄 경우 이상한 값이 들어가게 됨.
