...
//입력이 완료될 경우 첫째 줄에는 평균을 츌력합니다.
//둘째 줄에서는 중앙 값을 출력합니다.

//평균은 주어진 모든 수의 합/ 수의 개수로 표현합니다.
//중앙 값은 주어진 수를 크기 순서대로 정리햤을 경우, 가장 중앙에 있는 값을 의미
	...
	{   //전체 사이즈보다 1 작은 범위부터 시작(처음 값과 다음 값을 비교하기 때문)
		for (int j = 0; j < (size - 1) - i; j++)
		{
			...
			if (numbers[j] > numbers[j + 1])
			{
				...
				numbers[i] = numbers[i + 1]; //다음 값 저장
				numbers[i + 1] = temp; //기존 값 넘겨붐
			}
		}
	}
...
int main()
{
	...
	int sum_value = sum(numbers, 5); //배열의 합 = sum(베열,배열개수);
	int avg_value = avg(sum_value, 5); //배열의 평군 = avg(합,배열개수);
