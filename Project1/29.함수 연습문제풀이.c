...
void Backtracking(int N, int M, int depth, int path[])
{
	...
	for (int i = 1; i <= M; i++)
	{
		...
		if (!used[i])
		{
			...
			//depth(진행 순서)의 값을 1 증가시켜서, 다시 함수(Backtraking)을 호출하는 작업을 진행합니다.
...
int main()
{
	...
	//헤당 문제의 작업 순서
	//1. scacf에 의해 N과 M이 결정됩니다. ex) 3 2
	...
	//   M과 depth가 동일하기 떄문에 전체 출력하고 작업 종료    결과 : 1 2 출력
	//   used[2] = 0
