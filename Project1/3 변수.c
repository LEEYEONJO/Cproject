...
//C언어에서 변수는 변수마다 정해진 데이터의 형태(자료형)아 존재하며, 각 변수는
//할당된 값을 가지고 있습니다.

//자료형(Type)
//프로그램어 데이터를 판단하는 기준, 데이터의 형태를 의미합니다.

//대표적인 자료형
//이름  표현하는 형태  크기    표현 범위
//int       정수      4 byte  -2147483648 ~ 217483647
//float     실수      4 byte   4 * 10 -38제곱 ~ 3.4 * 10 +38제곱  --> 소수점 이하 6자리까지 정확하게 표현
...
int main()
{
	...
	printf("16진수 표기법 출력(8자리) hexa : %o.8x\n", hexa);
	return 0;
}